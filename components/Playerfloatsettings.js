    ...
    if (getParameterByName("ty", document.location.href) == "tv") {
      (async () => {
        var { data } = await axios.get(
          `/api/getEpisodes?imdb=${getParameterByName(
            "d",
            document.location.href
          )}&tmdb=${getParameterByName("tmdb")}&s=all`
        );
        console.log(data);
        setmediadata(data);
      })();
            ...
            <div
              onClick={(e) => {
                if (!allserverofaml[0]) {
                  ...
                  if (
                    e.currentTarget.lastElementChild.classList.contains(
                      "hidden"
                    )
                  ) {
              ...
              className={`p-4 z-11 text-[12px] gap-2 flex-col md:text-[13px] cursor-pointer rounded-2xl items-center justify-center flex   ${
                allserverofaml[0] || serveramlspinner
                  ? "min-h-[6.5rem] md:min-h-[6.8rem]"
                  : "h-[8rem] md:h-[8rem]"
          ...
          {mediadata ? (
            <div>
                    ...
                    >
                      <div
                        className={`p-2 hover:bg-[rgba(203,213,225,0.8)] ${
                          ind == currentselectedseason
                            ? "bg-[rgba(203,213,225,0.8)]"
                            : ""
              ...
              <div
                ...
                // className="overflow-x-scroll"
              >
                {mediadata[currentselectedseason]?.episodes?.map(
                  (item, indde) => {
                    return (
                      // <div></div>
                      <div
                        ...
                        onClick={() => {
                          item.streamable
                            ? (window.location.href = `/SupportPlayer?d=${getParameterByName(
                                "d"
                              )}&t=${getParameterByName(
                                "t"
                              )}&ty=${getParameterByName(
                                "ty"
                              )}&w=${getParameterByName("w")}&s=${
                                item.episodeDetails.season_number
                              }&e=${
                                item.episodeDetails.episode_number
                              }&tmdb=${getParameterByName("tmdb")}`)
                            : null;
                          // ? router.push({
