...
const handler = nc({
  ...
  .get(async (req, res) => {
    try {
      ...
      let pathParams = req.query.value;
      let url = `https://${pathParams.join("/")}?${Object.keys(req.query)
        .filter((f) => f != "value")
        .map((i) => `${i}=${req.query[i]}`)
        .join("&")}`;

      if (req.url.includes(".m3u8")) {
        let replaceTraget = url.split(".m3u8")[0].split("/").reverse()[0];

        // return console.log(replaceTraget);
        var host = "https://lovesomecommunity.com/";
        var options = { headers: { Referer: host } };
        try {
          await axios.get(url, options).then((d) =>
            res.send(
              d.data.replaceAll(
                replaceTraget,
                `${req.url.split(".m3u8")[0]}`
                // req.url.split(".m3u8")
                //   "https://go2.gocast2.com:999/hls/star1in"
              )
            )
          );
        } catch (er) {
