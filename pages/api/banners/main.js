...
const handler = nc({
  ...
  (async () => {
    let { data } = await axios.post("https://apis.justwatch.com/graphql", {
      operationName: "GetGlobalLargeSpotlightTitles",
    ...
    // let banners = data.edges.map()

    let posters = data.data.popularTitles.edges.map(
      ({ node: { content } }) => ({
        // ...content,
        backdrops: content.backdrops.map(
          (i) =>
            "https://images.justwatch.com" +
            i.backdropUrl.replace("{profile}", "s640/.webp")
        ),
        banner: content.backdrops.map(
          (i) =>
            "https://images.justwatch.com" +
            i.backdropUrl.replace("{profile}", "s640/.webp")
        )[0],
        backdrop: content.backdrops.map(
          (i) =>
            "https://images.justwatch.com" +
            i.backdropUrl.replace("{profile}", "s640/.webp")
        )[0],
        poster:
          "https://images.justwatch.com" +
          content.posterUrl.replace("{profile}", "s166/.webp"),
        title: content.title,
        imdbRating: {
          score: content.scoring.imdbScore,
          votes: content.scoring.imdbVotes,
        },
        imdbId: content.externalIds.imdbId,
        imdb: content.externalIds.imdbId,
