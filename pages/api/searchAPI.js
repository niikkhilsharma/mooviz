  ...
  .get(async (req, res) => {
    try {
      var axres = await axios.get(
        `https://v3.sg.media-imdb.com/suggestion/x/${req.query.s}.json`
      );
      axres.data.d = axres.data.d.filter((el) =>
        el?.id.includes("tt") ? true : false
      );
      axres.data.d = axres.data.d.map((el) =>
        el?.id.includes("tt")
          ? {
              ...el,
              id: btoa(JSON.stringify(encrypthex(JSON.stringify(el.id)))),
            }
          : ""
      );
