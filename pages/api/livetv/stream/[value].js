...
const handler = nc({
  ...
  .get(async (req, res) => {
    try {
      ...
      if (!req.query.value.includes("live.php")) {
        await axios
          .get(
            `https://tv.googledrivelinks.com/autoq.php?c=${
              req.query.value.split(".")[0]
            }`
            //   options
          )
        ...
        await axios
          .get(
            `https://tv.googledrivelinks.com/${req.query.value.replace(
              ".m3u8",
              ""
            )}?c=${req.query.c}&q=${req.query.q}&e=.m3u8`
            //   options
          )
          .then((d) => res.send(d.data));
      }
