			...
			axios.get(`/api/getStream?imdb=${getParameterByName('d')}`).then(dat => {
				;(async () => {
					...
					if (!dat.data.streamable) {
						// router.push("/");
						var datasec2 = await axios.get(`/api/server4/getStream?imdb=${getParameterByName('d')}`)
						if (!datasec2.data.streamable) {
							...
							if (!onetimeemb) {
								...
								if (getParameterByName('ty') == 'tv') {
									ifrfor2em.src = `https://www.2embed.to/embed/imdb/tv?id=${getParameterByName('d')}&s=1&e=1`
								} else {
									ifrfor2em.src = `https://www.2embed.to/embed/imdb/movie?id=${getParameterByName('d')}`
								}
						...
						await axios
							...
							.then(m33 => setfullplayerconfig(dat => ({ ...dat, url: m33.data.data })))
					}
				})()
			})
		})()
								...
								<div className=" relative w-[100%]">
											...
											{fullplayerconfig.duration != 0
												? `${String(
														Math.floor(fullplayerconfig.duration / 3600)
															? Math.floor((fullplayerconfig.duration - fullplayerconfig.progess) / 3600)
															: '00'
												  ).padStart(2, '0')}:${String(
														Math.floor(((fullplayerconfig.duration - fullplayerconfig.progess) % 3600) / 60)
												  ).padStart(2, '0')}:${String(
														Math.floor(
															((fullplayerconfig.duration - (fullplayerconfig.progess % 3600)) % 60) % 60
														)
												  ).padStart(2, '0')}`
												: '--:--:--'}
										</h3>
									...
									<div className=" flex items-center justify-between pb-[25px] pt-[15px] px-[50px] flex-row w-[100%] ">
										<div className="flex">
											...
											>
												<img className="w-8 h-8   " src="/svg/o-backward.svg" alt="" />
											...
											>
												<h3 style={{ fontFamily: 'Rubik' }}>10s</h3>
												<img className="w-8 h-8  " src="/svg/o-forward.svg" alt="" />
											</div>
