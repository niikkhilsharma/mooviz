...
const GetUser = () => {
	...
	const [current_user_Country_Code, setcurrent_user_Country_Code] = useState('')
	useEffect(() => {
		var getUserCountry = require('js-user-country').default()
		var filter = codes.map(e => e.country)
		setcurrent_user_Country_Code('+' + codes[filter.indexOf(getUserCountry.name)].countryCodes[0])
	}, [$.ready])
	...
	async function signupsubmit(data) {
		...
		try {
			...
			if (resultsgetotp.accepted && resultsgetotp.msg == 'SMS_SENT') {
				...
				$('.contactsignupcl').parent().children().last('span').removeClass('hidden')
				$('.contactsignupcl').parent().children().last('span').removeAttr('style')
				$('.contactsignupcl').next('label').text('Enter OTP')
				$('.contactsignupcl').val('')
				setcansignup(true)
				setalreadygototponsignup(true)
				$('#signupform').children().last('div').children().first().text('SUBMIT')
				// user.saveuserinfo();
				// navi("/", { replace: true });
				// router.push("/");
			} else {
	...
	function verifylogin(e, callback) {
		if (/^\d+$/.test($('.emailinplogin').val().replace('+', ''))) {
			...
			$('.spannerrlogin').hide()
			if ($('.emailinplogin').val().length >= 10 && $('.passinplogin').val().length > 7) {
				callback(datatosend)
			...
			$('.spannerrlogin').hide()
			if (checkemail($('.emailinplogin').val()) && $('.passinplogin').val().length > 7) {
				callback(datatosend)
				//   loginsubmit(datatosend);
						...
						<div className="relative md:w-[26rem] w-auto h-auto md:h-[28rem] justify-between flex flex-col">
							<div className="hidden md:block pt-[12%]">
								...
								<div className="mt-[13px]">
									<h3 className="text-[1.8rem] text-center">{`${CurrentuserState} Movizz`}</h3>
									<h3 className="text-[1rem] text-center">& Access 99M+ Collection</h3>
								</div>
							</div>
              ...
              /> */}
                ...
                <h3 className="text-[1.2rem] ml-[4.7rem] w-[20rem]">
                  Access 99M+ Collection
						...
						>
							<div className="user-box flex">
								<input
									...
									onChange={e => {
										if (e.currentTarget.value.length >= 10 && /^\d+$/.test(e.currentTarget.value)) {
											if (!e.currentTarget.value.includes(current_user_Country_Code)) {
												if (
													e.currentTarget.value.includes(
														current_user_Country_Code.substring(0, current_user_Country_Code.length - 1)
													)
												) {
													e.currentTarget.value =
														e.currentTarget.value + current_user_Country_Code.split('').at(-1)[0]
												} else {
													e.currentTarget.value = current_user_Country_Code + e.currentTarget.value
												}
											} else {
							...
							<div className="user-box flex">
								<input type="password" name="" required className="passinplogin" />
					...
					{/*For Signup Screen */}
					<div className="w-auto md:w-[50%] login-box hidden" style={{ padding: '11px 40px' }}>
						<h2>Signup</h2>
						<form
							id="signupform"
							onSubmit={e => {
						...
						>
							...
							<div className="user-box flex group">
								...
								<input
									...
									onChange={e => {
										if (e.currentTarget.value.length >= 10) {
											...
											e.currentTarget.previousElementSibling.classList.add('flex')
											// e.currentTarget.value =
								...
								<span className=" w-[105%] !text-[13px] gap-[11px] text-red-500 absolute mt-11 hidden spannerrsignup">
									...
									<h5 onClick={gotogetotpsignup} className="cursor-pointer w-full text-blue-600">
										{' Change Number'}
									</h5>
								</span>
							</div>
							<div className="user-box flex">
								<input type="password" name="" required className="passwordsignupcl" />
								<label>Password</label>
								<span className=" text-red-500 absolute mt-11 hidden spannerrsignup">
									<h5>Must Be More Than 7 Digits</h5>
								</span>
